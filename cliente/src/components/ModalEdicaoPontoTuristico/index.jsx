...
export const ModalEdicaoPontoTuristico = ({
    id,
    nomeAtual,
    descricaoAtual,
    enderecoAtual,
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            if (respostaEstados.status == 200) {
                ...
                const estadoDoPontoTuristico = respostaEstados.data.filter(estadoData => estadoData.nome == estadoAtual);
                setEstadoId(estadoDoPontoTuristico[0].id)
            }
        }
    ...
    async function editarPontoTuristico() {
        try {
            await axios.put(`http://localhost:8080/pontos-turisticos/${id}`,
                {
                    "nome": nome,
                    "descricao": descricao,
                    "endereco": endereco,
    ...
    return (
        <div className={styles.modal}>
            ...
            <button className={styles.btnCadastrar} onClick={editarPontoTuristico}>Cadastrar</button>
        </div>
    )
}